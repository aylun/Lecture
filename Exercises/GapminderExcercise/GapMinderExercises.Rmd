...
filter(gapminder, year == 1957) %>% arrange(desc(pop))
```
...
```{r}
# Filter, mutate, and arrange the gapmilifeExpMonths
gapminder %>% 
  filter(year == 2007) %>%
...
* Create a scatter plot with population (`pop`) on the x-axis and GDP per capita (`gdpPercap`) on the y-axis. Put both the x- and y- axis on a log scale.
...
ggplot(gapminder, aes(pop,lifeExp,color = continent,size = gdpPercap))+
  geom_point()+  
  scale_x_log10()
```
